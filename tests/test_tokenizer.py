...
def test_tokenizes_identifiers() -> None:
    correct_results = create_tokens(
        "identifier", "if", "while", "_var1", "Ok_2")
...
def test_tokenizes_operators() -> None:
    correct_result = create_tokens(
        "operator", "+", "-", "*", "/", "%", "=", "==", "!=",
        "<", "<=", ">", ">="
    )
...
def test_tokenizes_punctuation() -> None:
    correct_result = create_tokens("punctuation", "(", "{", "}", ")", ",", ";")
    assert tokenizer("({} ),;") == correct_result


def test_tokenizer_ignores_comments() -> None:
